...
class ChatGroupListState extends State<ChatGroupListPage> {
          ...
          if (googleInfoState is GoogleInfoLoaded) {
            return BlocBuilder<UserBloc, UserState>(
              builder: (context, userState) {
                ...
                if (userState is UserLoaded) {
                  return BlocBuilder<ConversationGroupBloc,
                    ...
                    builder: (context, conversationGroupState) {
                      ...
                      if (conversationGroupState is ConversationGroupsLoaded) {
                        return BlocBuilder<UnreadMessageBloc,
                            UnreadMessageState>(
                          builder: (context, unreadMessageState) {
                            ...
                            if (unreadMessageState is UnreadMessagesLoaded) {
                              return BlocBuilder<MultimediaBloc,
                                  MultimediaState>(
                                builder: (context, multimediaState) {
                                  ...
                                  if (multimediaState is MultimediaLoaded) {
                                    if (isObjectEmpty(conversationGroupState
                                      ...
                                      return Center(
                                          child: Text(
                                              "No conversations. Tap \"+\" to create one!"));
                                    } else {
                                      return SmartRefresher(
                                        ...
                                        child: ListView.builder(
                                            ...
                                            shrinkWrap: true,
                                            itemCount: conversationGroupState
                                                .conversationGroupList.length,
                                  ...
                                  return Center(
                                      child: Text(
                                          'Error. Multimedia are not loaded.'));
                                },
                              );
                            }
                            ...
                            return Center(
                                child: Text(
                                    'Error. Unread Messages are not loaded.'));
                          },
                        );
                      }
                      ...
                      return Center(
                          child: Text(
                              'Error. Conversation Groups are not loaded.'));
                    },
                  );
                }
    ...
    return PageListItem(
        ...
        onTap: (BuildContext context, object) {
          ...
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: ((context) => ChatRoomPage(conversationGroup))));
        });
  }
  ...
  initialize(BuildContext context) async {
    BlocProvider.of<MultimediaProgressBloc>(context)
        .add(InitializeMultimediaProgressEvent(callback: (bool done) {}));
    BlocProvider.of<ConversationGroupBloc>(context)
        .add(InitializeConversationGroupsEvent(callback: (bool done) {}));
    BlocProvider.of<MessageBloc>(context)
        .add(InitializeMessagesEvent(callback: (bool done) {}));
    BlocProvider.of<MultimediaBloc>(context)
        .add(InitializeMultimediaEvent(callback: (bool done) {}));
    BlocProvider.of<UnreadMessageBloc>(context)
        .add(InitializeUnreadMessagesEvent(callback: (bool done) {}));
    BlocProvider.of<UserContactBloc>(context)
        .add(InitializeUserContactsEvent(callback: (bool done) {}));
    BlocProvider.of<IPGeoLocationBloc>(context)
        .add(InitializeIPGeoLocationEvent(callback: (bool done) {}));
  }
  ...
  processWebSocketMessage(
    ...
    webSocketStream.listen((data) {
      ...
      BlocProvider.of<WebSocketBloc>(context)
          .add(ReconnectWebSocketEvent(user: user, callback: (bool done) {}));
    }, onDone: () {
