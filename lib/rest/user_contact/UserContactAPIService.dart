...
class UserContactAPIService {
  ...
  Future<UserContact> addUserContact(UserContact userContact) async {
    ...
    var httpResponse = await http.post(wholeURL,
        body: userContactJsonString, headers: createAcceptJSONHeader());
  ...
  Future<bool> editUserContact(UserContact userContact) async {
    String userContactJsonString = json.encode(userContact.toJson());
    return httpResponseIsOK(await http.put("$REST_URL/$userContactAPI",
        body: userContactJsonString, headers: createAcceptJSONHeader()));
  }

  Future<bool> deleteUserContact(String userContactId) async {
    return httpResponseIsOK(
        await http.delete("$REST_URL/$userContactAPI/$userContactId"));
  }

  Future<UserContact> getUserContact(String userContactId) async {
    return getUserContactBody(
        await http.get("$REST_URL/$userContactAPI/$userContactId"));
  }

  Future<UserContact> getUserContactByMobileNo(String mobileNo) async {
    return getUserContactBody(
        await http.get("$REST_URL/$userContactAPI/mobileNo/$mobileNo"));
  }

  Future<List<UserContact>> getUserContactsByUserId(String userId) async {
    return getUserContactListBody(
        await http.get("$REST_URL/$userContactAPI/user/$userId"));
  }

  UserContact getUserContactBody(Response httpResponse) {
    if (httpResponseIsOK(httpResponse)) {
      UserContact userContact =
          new UserContact.fromJson(json.decode(httpResponse.body));
      return userContact;
    }
    return null;
  }
  ...
  List<UserContact> getUserContactListBody(Response httpResponse) {
    if (httpResponseIsOK(httpResponse)) {
      ...
      List<UserContact> userContactList =
          list.map((model) => UserContact.fromJson(model)).toList();
