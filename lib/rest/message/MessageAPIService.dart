...
class MessageAPIService {
  ...
  Future<ChatMessage> addMessage(ChatMessage message) async {
    ...
    var httpResponse = await http.post(wholeURL,
        body: messageJsonString, headers: createAcceptJSONHeader());
  ...
  Future<bool> editMessage(ChatMessage message) async {
    String messageJsonString = json.encode(message.toJson());
    return httpResponseIsOK(await http.put("$REST_URL/$messageAPI",
        body: messageJsonString, headers: createAcceptJSONHeader()));
  }
  ...
  Future<List<ChatMessage>> getMessagesOfAConversation(
      String conversationId) async {
    return getMessageListBody(
        await http.get("$REST_URL/$messageAPI/conversation/$conversationId"));
  }

  ChatMessage getMessageBody(Response httpResponse) {
    if (httpResponseIsOK(httpResponse)) {
      ChatMessage message =
          new ChatMessage.fromJson(json.decode(httpResponse.body));
      return message;
    }
    return null;
  }
