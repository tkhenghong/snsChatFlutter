...
class UserAPIService {
  ...
  Future<User> addUser(User user) async {
    ...
    var httpResponse = await http.post(wholeURL,
        body: userJsonString, headers: createAcceptJSONHeader());
  ...
  Future<bool> editUser(User user) async {
    String userJsonString = json.encode(user.toJson());
    return httpResponseIsOK(await http.put("$REST_URL/$userAPI",
        body: userJsonString, headers: createAcceptJSONHeader()));
  }
  ...
  Future<User> getUserByUsingGoogleAccountId(String googleAccountId) async {
    return getUserBody(
        await http.get("$REST_URL/$userAPI/googleAccountId/$googleAccountId"));
  }
