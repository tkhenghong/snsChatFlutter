...
class SettingsAPIService {
  ...
  Future<Settings> addSettings(Settings settings) async {
    ...
    var httpResponse = await http.post("$REST_URL/$settingsAPI", body: settingsJsonString, headers: createAcceptJSONHeader());
  ...
  Future<bool> editSettings(Settings settings) async {
    String settingsJsonString = json.encode(settings.toJson());
    return httpResponseIsOK(await http.put("$REST_URL/$settingsAPI", body: settingsJsonString, headers: createAcceptJSONHeader()));
  }

  Future<bool> deleteSettings(String settingsId) async {
    return httpResponseIsOK(await http.delete("$REST_URL/$settingsAPI/$settingsId"));
  }

  Future<Settings> getSingleSettings(String settingsId) async {
    return getSettingsBody(await http.get("$REST_URL/$settingsAPI/$settingsId"));
  }

  Future<Settings> getSettingsOfAUser(String userId) async {
    return getSettingsBody(await http.get("$REST_URL/$settingsAPI/user/$userId"));
  }
